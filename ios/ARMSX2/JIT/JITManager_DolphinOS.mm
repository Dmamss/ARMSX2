...
- (BOOL)initializeLuckTXM {
    ...
    // Allocate R/X region
    void* rx_ptr = mmap(NULL, regionSize, PROT_READ | PROT_EXEC,
...
- (void *)allocateLuckTXM:(size_t)size {
    ...
    dispatch_sync(self.jitQueue, ^{
        ...
        if (self.luckTXM_allocated + size > self.luckTXM_region_size) {
            NSLog(@"[ARMSX2-JIT-DolphinOS] LuckTXM region full (%zu / %zu bytes)",
                  self.luckTXM_allocated, self.luckTXM_region_size);
            return;
        }
        ...
        JITAllocation* alloc = [[JITAllocation alloc] initWithRXPointer:rx_ptr
                                                              RWPointer:rw_ptr
                                                                   size:size];
        ...
        NSLog(@"[ARMSX2-JIT-DolphinOS] LuckTXM allocated %zu bytes at %p (total: %zu)",
              size, rx_ptr, self.luckTXM_allocated);
    });
...
- (void *)allocateLuckNoTXM:(size_t)size {
    ...
    dispatch_sync(self.jitQueue, ^{
        ...
        NSLog(@"[ARMSX2-JIT-DolphinOS] LuckNoTXM allocated %zu bytes at %p/%p",
              size, rx_ptr, rw_ptr);
    });
...
- (JITAllocation *)getAllocationInfo:(void *)rx_ptr {
    ...
    dispatch_sync(self.jitQueue, ^{
        alloc = self.allocations[[NSValue valueWithPointer:rx_ptr]];
    });
